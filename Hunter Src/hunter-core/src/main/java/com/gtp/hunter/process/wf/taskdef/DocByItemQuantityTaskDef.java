...
public class DocByItemQuantityTaskDef extends BaseTaskDef {

	private transient static final Logger logger = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

	public DocByItemQuantityTaskDef(TaskDef def, RegisterService regSvc, RegisterStreamManager rsm, User usr) {
	...
	protected List<Document> listQuickDocuments(User usr) {
		return getRegSvc().getDcSvc().listById(getRegSvc()
						.getTskSvc()
						.listStubsByModelAndItemThingDifference(getDef().getModel())
						.stream()
						.map(vts -> vts.getId())
