...
public class AllTasksWebSocketServer {
	...
	public void open(@PathParam("token") String token, Session session) {
		if (regSvc.getAuthSvc().valid(token)) {
			logger.info("CONECTANDO USERTASKS " + token + " (" + session.getId() + ": " + session.getOpenSessions().size() + ")");
			session.getUserProperties().put(TOKEN_KEY, token);
			TaskSession ts = new TaskSession(session);
			tsm.subscribeByUser(token, ts);
	...
	public void close(Session ss, CloseReason cr) {
		logger.info("FECHANDO USERTASKS (" + cr.getCloseCode() + " - " + cr.getReasonPhrase() + "): " + ss.getUserProperties().get(TOKEN_KEY) + " (" + ss.getId() + ": " + ss.getOpenSessions().size() + ")");
