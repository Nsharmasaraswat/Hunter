...
public class DocByModelAndStatusParentFieldParam extends BaseTaskDef {

	private transient static final Logger	logger	= LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

	private String							taskDefMeta;

	public DocByModelAndStatusParentFieldParam(TaskDef def, RegisterService regSvc, RegisterStreamManager rsm, User usr) {
	...
	protected List<Document> listQuickDocuments(User usr) {
		...
		List<Document> lst = getRegSvc().getDcSvc().quickListByTypeStatusParentFieldValue(getDef().getModel().getMetaname(), getDef().getState(), meta, value);
	...
	protected List<ViewTaskStub> validateTask(Document d) {
		...
		if (d.getModel().getMetaname().equals(getDef().getModel().getMetaname())) {
			if (d.getStatus().equals(getDef().getState())) {
				Document parent = getRegSvc().getDcSvc().findParent(d.getId());
				...
				if (parent != null) {
					...
					jsonReader.close();
					if (parent.getFields().stream().anyMatch(df -> df.getField().getMetaname().equals(meta) && df.getValue().equals(value)))
						ret.add(d);
				}
			}
		}
