...
export class PalletComponent implements AfterViewInit, OnChanges {
    @Input("model") things: HunterThing[];
    ...
    ngAfterViewInit(): void {
        ...
        for (let thing of this.things) {
            ...
            if (thing.siblings !== undefined) {
                if (thing.siblings.length > 0) {
                    thArr = thArr.concat(thing.siblings.map(th => this.getThingStub(id, th)));
                } else {
                    thArr.push(this.getThingStub(id, thing));
                }
            }
        }
    ...
    getThingStub(id: string, th: HunterThing) {
        ...
        let addr_name: string = th.address === undefined ? 'EXPEDIDO' : th.address.name;
