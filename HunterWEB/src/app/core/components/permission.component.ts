...
export class PermissionComponent implements OnInit {
    ...
    loadData() {
        this.http.get(environment.coreserver + 'permission/' + this.id)
            .subscribe((msg: any) => {
                console.log(msg);
                this.data = msg;
            }, error => {
    ...
    onSave() {
        ...
        this.http.post(environment.coreserver + 'permission', this.data)
            .subscribe((msg: any) => {
                console.log(msg);
                if (msg != null) {
                    this.msgSvc.add({ severity: 'success', summary: 'DATA SAVED', detail: 'Data saved to database' });
                    if (this.id === '0')
                        this.router.navigate(['home/core/permission/' + msg.id]);
                    else
                        this.refresh();
                } else {
    ...
    getHeader() {
        return this.id === '0' ? 'New Permission' : 'Editing Permission';
    }

    backRoute() {
        // go to list page
        this.router.navigate(['home/core/permissionList/']);
    }
}
