...
export class LocationComponent implements OnInit {
    ...
    loadData() {
        this.http.get(environment.coreserver + 'location/' + this.id)
            .subscribe((msg: any) => {
                console.log(msg);
                this.data = msg;
            }, error => {
    ...
    onSave() {
        this.http.post(environment.coreserver + 'location', this.data)
            .subscribe((msg: any) => {
                console.log(msg);
                if (msg != null) {
                    this.msgSvc.add({ severity: 'success', summary: 'DATA SAVED', detail: 'Data saved to database' });
                    if (this.id === '0')
                        this.router.navigate(['home/core/location/' + msg.id]);
                    else
                        this.refresh();
                } else {
    ...
    getHeader() {
        return this.id === '0' ? 'New Location' : 'Editing Location';
    }

    backRoute() {
        // go to list page
        this.router.navigate(['home/core/locationList/']);
    }
}
