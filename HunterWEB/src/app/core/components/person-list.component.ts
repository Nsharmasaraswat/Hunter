...
export class PersonListComponent implements OnInit {
    ...
    loadData() {
        this.http.get(environment.coreserver + 'person/all')
            .subscribe((msg: any) => {
                this.data = msg.map(s => {
                    s.createdAtText = this.formatDate(new Date(s.createdAt));
                    s.birthdate = this.formatDate(new Date(s.birthdate));
                    return s;
                });
    ...
    onDelete() {
        if (this.selected == null) {
            this.msgSvc.add({ severity: 'error', summary: 'PLEASE SELECT AN ITEM', detail: 'A selection is necessary to perform the action' });
            return;
        }

        this.http.delete(environment.coreserver + 'person/' + this.selected.id, { responseType: 'text' })
            .subscribe(msg => {
                if (msg === 'Ok') {
    ...
    onNew() {
        ...
        this.router.navigate(['home/core/person/0']);
    }
    ...
    onEdit() {
        // go to create/editing page 
        this.router.navigate(['home/core/person/' + this.selected.id]);
    }
