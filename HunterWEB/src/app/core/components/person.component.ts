...
export class PersonComponent implements OnInit {
    ...
    loadData() {
        this.http.get(environment.coreserver + 'person/' + this.id)
            .subscribe((msg: any) => {
                console.log(msg);
                this.data = msg;
    ...
    onSave() {
        this.http.post(environment.coreserver + 'person', this.data)
            .subscribe((msg: any) => {
                console.log(msg);
                if (msg != null) {
                    this.msgSvc.add({ severity: 'success', summary: 'DATA SAVED', detail: 'Data saved to database' });
                    if (this.id === '0')
                        this.router.navigate(['home/core/person/' + msg.id]);
                    else
                        this.refresh();
                } else {
    ...
    getHeader() {
        return this.id === '0' ? 'New Person' : 'Editing Person';
    }

    backRoute() {
        // go to list page
        this.router.navigate(['home/core/personList/']);
    }
}
