...
export class PurposeListComponent implements OnInit {
    ...
    loadData() {
        this.http.get(environment.processserver + 'purpose/all')
            .subscribe((msg: any) => {
    ...
    onDelete() {
        ...
        this.http.delete(environment.processserver + 'source/' + this.selected.id, { responseType: 'text' })
            .subscribe(msg => {
                if (msg === 'Ok') {
    ...
    onNew() {
        ...
        this.router.navigate(['home/process/purpose/0']);
    }
    ...
    onEdit() {
        // go to create/editing page 
        this.router.navigate(['home/process/purpose/' + this.selected.id]);
    }
