...
export class ActionListComponent implements OnInit {
    ...
    loadData() {
        this.http.get(environment.processserver + 'action/all')
            .subscribe((msg: any) => {
                console.log(msg);
                this.data = msg.map(d => {
    ...
    onDelete() {
        ...
        this.http.delete(environment.processserver + 'action/' + this.selected.id, { responseType: 'text' })
            .subscribe(msg => {
                if (msg === 'Ok') {
    ...
    onNew() {
        ...
        this.router.navigate(['home/process/action/0']);
    }

    onEdit() {
        // go to create/editing page 
        this.router.navigate(['home/process/action/' + this.selected.id]);
    }
