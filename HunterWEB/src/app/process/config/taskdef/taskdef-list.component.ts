...
@Component({
    templateUrl: './taskdef-list.component.html'
})
export class TaskDefListComponent implements OnInit {
    ...
    onDelete() {
        ...
        this.http.delete(environment.processserver + 'taskdef/' + this.selected.id, { responseType: 'text' })
            .subscribe(msg => {
                if (msg === 'Ok') {
    ...
    onNew() {
        ...
        this.router.navigate(['home/process/taskdef/0']);
    }

    onEdit() {
        // go to create/editing page 
        this.router.navigate(['home/process/taskdef/' + this.selected.id]);
    }
