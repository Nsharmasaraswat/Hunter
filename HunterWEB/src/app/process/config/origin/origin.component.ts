...
export class OriginComponent implements OnInit {
    ...
    loadData() {
        this.http.get(environment.processserver + 'origin/' + this.id)
            .subscribe((msg: any) => {
                console.log(msg);
                this.data = msg;
            }, error => {
    ...
    onSave() {
        this.http.post(environment.processserver + 'origin', this.data)
            .subscribe((msg: any) => {
                console.log(msg);
                if (msg != null) {
                    this.msgSvc.add({ severity: 'success', summary: 'DATA SAVED', detail: 'Data saved to database' });
                    if (this.id === '0')
                        this.router.navigate(['home/process/origin/' + msg.id]);
                    else
                        this.refresh();
                } else {
    ...
    getHeader() {
        return this.id === '0' ? 'New Origin' : 'Editing Origin';
    }

    backRoute() {
        // go to list page
        this.router.navigate(['home/process/originList/']);
    }
}
