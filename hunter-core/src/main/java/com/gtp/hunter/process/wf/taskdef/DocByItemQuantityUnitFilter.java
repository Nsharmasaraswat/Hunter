...
public class DocByItemQuantityUnitFilter extends BaseTaskDef {

	private transient static final Logger logger = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

	public DocByItemQuantityUnitFilter(TaskDef def, RegisterService regSvc, RegisterStreamManager rsm, User usr) {
	...
	protected List<Document> listQuickDocuments(User usr) {
		return getRegSvc().getDcSvc().listById(getRegSvc()
						.getTskSvc()
						.listStubsByModelAndItemThingDifferenceAndUnit(getDef().getModel(), usr.getProperties().get("UNIT"))
						.stream()
						.map(vts -> vts.getId())
