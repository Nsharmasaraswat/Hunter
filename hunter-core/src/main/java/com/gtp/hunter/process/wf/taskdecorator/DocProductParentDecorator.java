...
public class DocProductParentDecorator extends BaseTaskDecorator {

	public DocProductParentDecorator(String params, RegisterService rSvc) {
		super(params, rSvc);
	}
	...
	public String decorateName(Document d) {
		...
		if (d.getParent() != null) {//LAZY PROXY INITIALIZATION FAILURE
			Document parent = getRegSvc().getDcSvc().findById(getRegSvc().getDcSvc().quickFindParentDoc(d.getId()).getId());
