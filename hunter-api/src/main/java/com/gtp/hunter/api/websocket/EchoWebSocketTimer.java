...
public class EchoWebSocketTimer  extends TimerTask {

	private Session ss;
	
	private String[] lstResposta = {
			"clock speed",
			"solar flares",
	...
	public EchoWebSocketTimer(Session ss) {
		this.ss = ss;
	}
	...
	public void run() {
		try {
			if(ss.isOpen()) {
				ss.getBasicRemote().sendText(lstResposta[new Double(Math.ceil(Math.random() * lstResposta.length -1)).intValue()]);
			} else {
				this.cancel();
			}
		} catch (IOException e) {
