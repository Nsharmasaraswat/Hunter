...
import com.gtp.hunter.vision.FrameMetadata;
...
public class ImageUtil {
    ...
    public static final double MAP_WIDTH = 98.075 * 200;
    public static final double MAP_HEIGHT = 84.610 * 200;

    public static Bitmap decodeCroppedBitmapFromResource(Resources res, int resId, double posX, double posY, int reqWidth, int reqHeight) {
        try {
            ...
            double relPosX = (posX + (MAP_WIDTH / 2)) / MAP_WIDTH;
            double relPosY = ((posY * -1) + (MAP_HEIGHT / 2)) / MAP_HEIGHT;
            int centerX = (int) Math.round(width * relPosX);
            int centerY = (int) Math.round(height * relPosY);
            ...
            return bitmapRegionDecoder.decodeRegion(new Rect(centerX - reqWidth, centerY - reqHeight, centerX + reqWidth, centerY + reqHeight), options);
        } catch (IOException ioe) {
            Log.e("Crop", ioe.getLocalizedMessage(), ioe);
        }
        return null;
    }
    ...
    public static Bitmap getBitmap(ImageProxy image) {
        ...
        ByteBuffer nv21Buffer = yuv420ThreePlanesToNV21(
                Objects.requireNonNull(image.getImage()).getPlanes(), image.getWidth(), image.getHeight());
