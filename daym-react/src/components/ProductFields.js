...
class ProductFields extends Component {
    ...
    checkPermission(permissions){
        return permissions.filter((each)=>each.route === '/product-field').length > 0;
    }

    componentDidMount() {
        let permissions = JSON.parse(localStorage.getItem('daym-user')).permissions;
        if(this.checkPermission(permissions)){
            this.setState({methods: permissions.filter((each)=>each.route === '/product-field')[0].methods});
        }else {
            window.location = '#/accessdenied';
        }
        const userId = this.props.location.pathname.split('/')[2];
        this.setState({userId});
        this.productFieldService.getAllFields(userId).then(response => {
    ...
    handleDeleteUserField(rowData){
        ...
        this.productFieldService.deleteProductField(rowData._id).then( response => {
    ...
    handleCreateUserField(){
        if(this.validate())
            ...
            if(this.state.editData){
                this.productFieldService.editProductField(this.state.editData._id,this.state.name,this.state.radioValue,this.state.chipsValue,this.state.userId,this.state.required).then(async (response)=>{
                ...
                this.productFieldService.createProductField(this.state.name,this.state.radioValue,this.state.chipsValue,this.state.userId,this.state.required).then(async (response) => {
                        ...
                        <div className="p-col-12 p-md-12">
                            ...
                            <div className="p-grid">
                                <div className="p-col-12 p-md-4">
                                    ...
                                    <label htmlFor="rb1" className="p-radiobutton-label">Text</label>
                                </div>
                                <div className="p-col-12 p-md-4">
                                    ...
                                    <label htmlFor="rb2" className="p-radiobutton-label">Number</label>
                                </div>
                                <div className="p-col-12 p-md-4">
                                    ...
                                    <label htmlFor="rb3" className="p-radiobutton-label">DropDown</label>
                                </div>
...
export default withTranslation()(ProductFields)
