...
class Users extends Component {
    ...
    constructor() {
        ...
        this.handleAddEditWarehouse = this.handleAddEditWarehouse.bind(this);
    ...
    checkPermission(permissions){
        return permissions.filter((each)=>each.route === '/users').length > 0;
    }

    componentDidMount() {
        let permissions = JSON.parse(localStorage.getItem('daym-user')).permissions;
        if(this.checkPermission(permissions)){
            this.setState({methods: permissions.filter((each)=>each.route === '/users')[0].methods});
        }else {
        ...
        this.userFieldService.getAllFields(userTypeId).then(response => {
            ...
            this.userService.getAllUser(userTypeId).then(res => {
    ...
    handleCreateWarehouse(){
        if(this.state.imageFile){
            this.appointmentService.uploadFile(this.state.imageFile).then((response)=>{
                if(response.data.result)
                    this.handleAddEditWarehouse(response.data.file);
                else
                    this.growl.show({
                        severity: 'error',
            ...
            this.handleAddEditWarehouse();
        }
    }
    handleAddEditWarehouse(avatar = null){
        const _avatar = avatar ? {avatar} : {};
        if(this.validate()) {
            ...
            if (this.state.editData) {
                ...
                this.userService.editUser(this.state.editData._id, {..._body,..._avatar}).then(async (response) => {
                  ...
                  //console.log('Position',position);
                    ...
                    this.growl.show({
                        ...
                        detail: this.props.t('user') + ' ' + this.props.t('edited_Successfully')
                    });
                    this.setState({
                    ...
                    let error = err.response.data.message;
                    this.growl.shotruew({ severity: 'error', summary: this.props.t('error'), detail: error || this.props.t('can_not_edit') + ' ' + this.props.t('user') });
              ...
              //console.log(_body);
                this.userService.createUser({..._body,..._avatar}).then(async (response) => {
                  //console.log('Resssss---->', response.data);
                    this.growl.show({
                        severity: 'success',
                        summary: this.props.t('success'),
                        detail: this.props.t('user') + ' ' + this.props.t('created_Successfully')
                    });
                    this.setState({
                    ...
                    let error = err.response.data.message;
                    this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: error || this.props.t('can_not_create') + ' ' + this.props.t('user') });
                    this.setState({buttonLoading: false});
                });
            }
        }
    ...
    handleDeleteWarehouse(rowData){
        ...
        this.userService.deleteUser(rowData._id).then( response => {
            ...
            this.growl.show({ severity: 'success', summary: this.props.t('success'), detail: this.props.t('user') + ' ' + this.props.t('deleted_Successfully') });
            this.setState({
                dataTableValue: _dataTable,
                deleteLoading: false,
            });
            ...
            let error = err.response.data.message;
            this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: error || this.props.t('can_not_delete') + ' ' + this.props.t('user') });
    ...
    render() {
        ...
        const tableHeader = (
            <div style={{display: 'flex',justifyContent: 'flex-end'}}>
                <Dialog header={(this.state.editData ? this.props.t('edit'): this.props.t('add'))+' '+this.props.t('user')} visible={this.state.display} modal={true} style={{width: '30vw'}} footer={dialogFooter} onHide={() => this.setState({display:false})}>
                    <div className="p-grid">
                        <div className="p-grid p-col-12 p-md-12">
                            <div className="p-col-12 p-md-12" style={{display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%', flexDirection: 'column'}}>
                                <img
                                    style={{width: 150, height: 150}}
                                    src={this.state.editData ? this.state.editData.avatar : this.state.src ? this.state.src : 'assets/layout/images/add-user.png'}
                                />
                                <label htmlFor="user-avatar" style={{cursor: 'pointer'}}>
                                    {this.props.t('choose_image')}
                                </label>
                                <input
                                    accept="image/*"
                                    hidden
                ...
                </Dialog>
                {
                    ...
                    <Button type="button" label={this.props.t('create_user')} icon="pi pi-cog"
        ...
        const viewDetails = (rowData) => <div>
            <Button label={this.props.t('view_user_detail')} className="p-button-info p-button-raised"  style={{marginRight: '.5em'}} onClick={()=>{
                ...
                <div className="p-grid" style={{height: '75vh'}}>
                    <div className="p-col-12" style={{height: '75vh'}}>
                        <h2 style={{
                            marginTop: '35vh',
                            textAlign: 'center'
                        }}>{this.props.t('no_user_category')}</h2>
                    </div>
                </div>
            );
        }
        return (
            <div className="p-grid">
                        ...
                        <h1>{this.props.t('users')}</h1>
                        {
                            this.state.loading ?
                                ...
                                <DataTable ref={(el) => this.dt = el} value={this.state.dataTableValue}
                                    ...
                                    <Column header={this.props.t('user_detail')}
                ...
                <Dialog header={this.props.t('user_detail')} visible={this.state.detailsShow} modal={true} style={{width: '40vw'}} footer={
                    <div>
                    ...
                    <div className="p-grid">
                        <div className="p-grid p-col-12 p-md-12">
                            <DataTable ref={(el) => this.dt = el} value={this.state.selectedFields} header={this.props.t('user_fields')} selectionMode="single" paginator={true} rows={7}
