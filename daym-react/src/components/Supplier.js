...
class Supplier extends Component {
    ...
    checkPermission(permissions){
        return permissions.filter((each)=>each.route === '/supplier').length > 0;
    }

    componentDidMount() {
        let permissions = JSON.parse(localStorage.getItem('daym-user')).permissions;
        if(this.checkPermission(permissions)){
            this.setState({methods: permissions.filter((each)=>each.route === '/supplier')[0].methods});
        }else {
            window.location = '#/accessdenied';
        }
        this.userService.getSupplierType().then(response =>{
                ...
                this.userService.editUser(this.state.editData._id, _body).then(async (response) => {
                  ...
                  //console.log('Position',position);
                    ...
                    this.growl.show({
                        ...
                        detail: this.props.t('supplier') + ' ' + this.props.t('edited_Successfully')
                    });
                    this.setState({
                    ...
                    let error = err.response.data.message;
                    this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: error || this.props.t('can_not_edit') + ' ' + this.props.t('supplier') });
                ...
                this.userService.createUser(_body).then(async (response) => {
                  //console.log('Resssss---->', response.data);
                    this.growl.show({
                        severity: 'success',
                        summary: this.props.t('success'),
                        detail: this.props.t('supplier') + ' ' + this.props.t('created_Successfully')
                    });
                    this.setState({
                    ...
                    let error = err.response.data.message;
                    this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: error || this.props.t('can_not_create') + ' ' + this.props.t('supplier') });
                    this.setState({buttonLoading: false});
                });
            }
        }
    ...
    handleDeleteWarehouse(rowData){
        ...
        this.userService.deleteUser(rowData._id).then( response => {
            ...
            this.growl.show({
                severity: 'success',
                summary: this.props.t('success'),
                detail: this.props.t('supplier') + ' ' + this.props.t('deleted_Successfully')
            });
            this.setState({
                dataTableValue: _dataTable,
                deleteLoading: false
            ...
            let error = err.response.data.message;
            this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: error || this.props.t('can_not_delete') + ' ' + this.props.t('supplier') });
    ...
    render() {
        ...
        const tableHeader = (
            <div style={{display: 'flex',justifyContent: 'flex-end'}}>
                    ...
                    }}/>
                {
                    ...
                    <Button type="button" label={this.props.t('create_supplier')} icon="pi pi-cog"
        ...
        const viewDetails = (rowData) => <div>
            <Button label={this.props.t('view') + ' ' + this.props.t('supplier_details')} className="p-button-info p-button-raised"  style={{marginRight: '.5em'}} onClick={()=>{
                this.setState({detailsShow: true, selectedFields: rowData.fields})
            }}/>
        </div>;
        const viewDriver = (rowData) => <div>
            <Button label={this.props.t('view') + ' ' + this.props.t('driver')} className="p-button-info p-button-raised"  style={{marginRight: '.5em'}} onClick={()=>{
                window.location = '#/driver/'+rowData._id;
            }}/>
        </div>;
        const viewTruck = (rowData) => <div>
            <Button label={this.props.t('view') + ' ' + this.props.t('truck')} className="p-button-info p-button-raised"  style={{marginRight: '.5em'}} onClick={()=>{
                window.location = '#/supplier-truck/'+rowData._id;
            }}/>
        </div>;
        ...
        return (
            <div className="p-grid">
                        ...
                        <h1>{this.props.t('suppliers')}</h1>
                        {
                            this.state.loading ?
                                ...
                                <DataTable ref={(el) => this.dt = el} value={this.state.dataTableValue}
                                    ...
                                    <Column header={this.props.t('driver')} body={(rowData) => viewDriver(rowData)}
                                            style={{textAlign: 'center'}}/>
                                    <Column header={this.props.t('view_trucks')} body={(rowData) => viewTruck(rowData)}
                                            style={{textAlign: 'center'}}/>
                                    <Column header={this.props.t('supplier_details')}
                ...
                <Dialog header={this.props.t('supplier_details')} visible={this.state.detailsShow} modal={true} style={{width: '40vw'}} footer={
                    <div>
                    ...
                    <div className="p-grid">
                        <div className="p-grid p-col-12 p-md-12">
                            <DataTable ref={(el) => this.dt = el} value={this.state.selectedFields} header={this.props.t('supplier_fields')} selectionMode="single" paginator={true} rows={7}
