...
class AllDriver extends Component {
    ...
    checkPermission(permissions){
        return permissions.filter((each)=>each.route === '/all-drivers').length > 0;
    }

    componentDidMount() {
        let permissions = JSON.parse(localStorage.getItem('daym-user')).permissions;
        if(this.checkPermission(permissions)){
            this.setState({methods: permissions.filter((each)=>each.route === '/all-drivers')[0].methods});
        }else {
            window.location = '#/accessdenied';
        }
        const parentId = this.state.user._id;
        this.setState({parentId});
        this.userService.getDriverType().then(response =>{
            ...
            this.userFieldService.getAllFields(userTypeId).then(response => {
                ...
                this.userService.getAllDrivers(parentId).then(res => {
            ...
            if (this.state.editData) {
                ...
                this.userService.editUser(this.state.editData._id, _body).then(async (response) => {
                    ...
                    this.growl.show({
                        ...
                        detail: this.props.t('driver') + ' ' + this.props.t('edited_Successfully')
                    });
                    this.setState({
                    ...
                    let error = err.response.data.message;
                    this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: error || this.props.t('can_not_edit') + ' ' + this.props.t('driver') });
                ...
                this.userService.createUser(_body).then(async (response) => {
                    ...
                    this.growl.show({
                        severity: 'success',
                        summary: this.props.t('success'),
                        detail: this.props.t('driver') + ' ' + this.props.t('created_Successfully')
                    });
                    this.setState({
                    ...
                    let error = err.response.data.message;
                    this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: error || this.props.t('can_not_create') + ' ' + this.props.t('driver') });
                    this.setState({buttonLoading: false});
                });
            }
        }
    ...
    handleDeleteWarehouse(rowData){
        ...
        this.userService.deleteUser(rowData._id).then( response => {
            ...
            this.growl.show({ severity: 'success', summary: this.props.t('success'), detail: this.props.t('driver') + ' ' + this.props.t('deleted_Successfully') });
            this.setState({
                dataTableValue: _dataTable,
                deleteLoading: false,
            });
            ...
            let error = err.response.data.message;
            this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: error || this.props.t('can_not_delete') + ' ' + this.props.t('driver') });
    ...
    render() {
        ...
        const tableHeader = (
            <div style={{display: 'flex',justifyContent: 'flex-end'}}>
                <Dialog header={(this.state.editData ? this.props.t('edit'): this.props.t('add'))+' '+this.props.t('driver')} visible={this.state.display} modal={true} style={{width: '30vw'}} footer={dialogFooter} onHide={() => this.setState({display:false})}>
                                    ...
                                    <div className="p-col-12 p-md-12" key={index}>
                                        {
                                            each.type === 'Select' ?
                                                ...
                                                <InputText placeholder={each.name} style={{width: '100%'}} type={each.type} value={this.state.userValues[index].value} onChange={(e) => {
                                                    let _values = this.state.userValues;
                                                    if(each.type === 'Number') {
                                                        _values[index].value = parseInt(e.target.value);
                                                    } else {
                ...
                </Dialog>
                {
                    ...
                    <Button type="button" label={this.props.t('create_driver')} icon="pi pi-cog"
        ...
        const viewDetails = (rowData) => <div>
            <Button label={this.props.t('view_driver_details')} className="p-button-info p-button-raised"  style={{marginRight: '.5em'}} onClick={()=>{
        ...
        return (
            <div className="p-grid">
                        ...
                        <h1>{this.props.t('drivers')}</h1>
                        {
                            this.state.loading ?
                                ...
                                <DataTable ref={(el) => this.dt = el} value={this.state.dataTableValue}
                                    ...
                                    <Column header={this.props.t('driver_details')}
                ...
                <Dialog header={this.props.t('driver_details')} visible={this.state.detailsShow} modal={true} style={{width: '40vw'}} footer={
                    <div>
                    ...
                    <div className="p-grid">
                        <div className="p-grid p-col-12 p-md-12">
                            <DataTable ref={(el) => this.dt = el} value={this.state.selectedFields} header={this.props.t('driver_fields')} selectionMode="single" paginator={true} rows={7}
                                       responsive={true} selection={this.state.selectedFields1} onSelectionChange={event => this.setState({selectedFields1: event.value})}>
                                <Column body={(row)=>row.userField.name} header={this.props.t('driver_fields')} style={{textAlign:'center'}}/>
...
export default withTranslation()(AllDriver)
