...
class ProductType extends Component {
    ...
    viewProducts(rowData, column) {
        return <div style={{padding: '0px 8px'}}>
            <Button label={this.props.t('view_product')} className="p-button-info p-button-raised"  style={{marginRight: '.5em'}} onClick={()=>{
                window.location = '#/products/'+rowData._id;
            }}/>
        </div>;
    }
    ...
    addFields(rowData, column) {
        return <div>
            <Button label={this.props.t('view_fields')} className="p-button-info p-button-raised"  style={{marginRight: '.5em'}} onClick={()=>{
                window.location = '#/product-field/'+rowData._id;
            }}/>
        </div>;
    }
    ...
    handleDeleteUserGroup(rowData){
        ...
        this.productTypeService.deleteProductType(rowData._id).then( response => {
            ...
            this.growl.show({ severity: 'success', summary: this.props.t('success'), detail: 'Product Type deleted Successfully!' });
            this.setState({
                dataTableValue: _dataTable,
                deleteLoading: false
            })
        }).catch((err)=>{
            this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: 'Can not delete Product Type' });
            this.setState({deleteLoading: false});
        });
    }

    checkPermission(permissions){
        return permissions.filter((each)=>each.route === '/product-type').length > 0;
    }

    componentDidMount() {
        let permissions = JSON.parse(localStorage.getItem('daym-user')).permissions;
        if(this.checkPermission(permissions)){
            this.setState({methods: permissions.filter((each)=>each.route === '/product-type')[0].methods});
        }else {
    ...
    validate(){
        ...
        if(!this.state.unloadingTime){
            this.growl.show({ severity: 'warn', summary: this.props.t('validation_failed'), detail: this.props.t('unloading_time_not_blank') });
            return false;
        }
        return true;
    }
    handleCreateUserGroup(){
        if(this.validate())
            ...
            if(this.state.editData){
                this.productTypeService.editProductType(this.state.editData._id,this.state.name,this.state.description,this.state.unloadingTime).then(async (response)=>{
                    this.growl.show({ severity: 'success', summary: this.props.t('success'), detail: 'Product Type edited Successfully!' });
                    let _dataTable = this.state.dataTableValue;
                    ...
                    this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: 'Can not edit Product Type' });
                    this.setState({buttonLoading: false});
                });
            }else {
                this.productTypeService.createProductType(this.state.name, this.state.description, this.state.unloadingTime).then(async (response) => {
    ...
    render() {
        const dialogFooter = (
            <div>
                <Button icon="pi pi-times" onClick={() => this.setState({display:false, name: '', description: '', editData: null})} label={this.props.t('cancel')} className="p-button-secondary" />
                {
        ...
        const tableHeader = (
            <div style={{display: 'flex',justifyContent: 'flex-end'}}>
                <Dialog header={(this.state.editData ? this.props.t('edit'): this.props.t('add'))+' '+this.props.t('product_category')} visible={this.state.display} modal={true} style={{width: '30vw'}} footer={dialogFooter}
                        onHide={() => this.setState({display:false, name: '', description: '', editData: null})}>
                    <div className="p-grid">
                        ...
                        <div className="p-col-12 p-md-12">
                            <InputText placeholder={this.props.t('unloading_time')+' '+this.props.t('in_min')} style={{width: '100%'}} value={this.state.unloadingTime} onChange={(e) => this.setState({unloadingTime: e.target.value})} type={'number'}/>
                        </div>
                    </div>
                </Dialog>
                {
                    this.state.methods.indexOf('create') >= 0 &&
                    <Button type="button" label={this.props.t('create_product_type')} icon="pi pi-cog"
            ...
            <div className="p-grid">
                ...
                <div className="p-col-12">
                    <div className="card card-w-title datatable-demo">
                        <h1>{this.props.t('product_category')}</h1>
                        {
                            this.state.loading ?
                                <div className="p-col-12"
                                ...
                                <DataTable ref={(el) => this.dt = el} value={this.state.dataTableValue}
                                    ...
                                    <Column field="unloadingTime" header={this.props.t('unloading_time')+' '+this.props.t('in_min')} sortable={true}/>
                                    <Column header={this.props.t('fields')} body={this.addFields}
                                            style={{textAlign: 'center'}}/>
                                    <Column header={this.props.t('product')} body={this.viewProducts}
                                            style={{textAlign: 'center'}}/>
