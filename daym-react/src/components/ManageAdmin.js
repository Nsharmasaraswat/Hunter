...
class ManageAdmin extends Component {
    ...
    checkPermission(permissions){
        return permissions.filter((each)=>each.route === '/manage-admins').length > 0;
    }
    ...
    componentDidMount() {
        ...
        if(this.checkPermission(permissions)){
            this.setState({methods: permissions.filter((each)=>each.route === '/manage-admins')[0].methods});
        }else {
            window.location = '#/accessdenied';
        }
        const locationId = this.props.location.pathname.split('/')[2];
        this.setState({locationId});
        this.userService.getAdminType().then(response =>{
            ...
            this.userService.getAllAdmins(userTypeId, locationId).then(res => {
                ...
                this.userService.editUser(this.state.editData._id, _body).then(async (response) => {
                  ...
                  //console.log('Position',position);
                    ...
                    this.growl.show({
                        ...
                        detail: this.props.t('admin') + ' ' + this.props.t('edited_Successfully')
                    });
                    this.setState({
                    ...
                    let error = err.response.data.message;
                    this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: error || this.props.t('can_not_edit') + ' ' + this.props.t('admin') });
                ...
                this.userService.createUser(_body).then(async (response) => {
                  //console.log('Resssss---->', response.data);
                    this.growl.show({
                        severity: 'success',
                        summary: this.props.t('success'),
                        detail: this.props.t('admin') + ' ' + this.props.t('created_Successfully')
                    });
                    this.setState({
                    ...
                    let error = err.response.data.message;
                    this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: error || this.props.t('can_not_create') + ' ' + this.props.t('admin') });
                    this.setState({buttonLoading: false});
                });
            }
        }
    ...
    handleDeleteWarehouse(rowData){
        ...
        this.userService.deleteUser(rowData._id).then( response => {
            ...
            this.growl.show({
                severity: 'success',
                summary: this.props.t('success'),
                detail: this.props.t('admin') + ' ' + this.props.t('deleted_Successfully')
            });
            this.setState({
                dataTableValue: _dataTable,
                deleteLoading: false,
            ...
            let error = err.response.data.message;
            this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: error || this.props.t('can_not_delete') + ' ' + this.props.t('admin') });
    ...
    render() {
        ...
        const tableHeader = (
            <div style={{display: 'flex',justifyContent: 'flex-end'}}>
                <Dialog header={(this.state.editData ? this.props.t('edit'): this.props.t('add'))+' '+this.props.t('admin')} visible={this.state.display} modal={true} style={{width: '30vw'}} footer={dialogFooter} onHide={() => this.setState({display:false})}>
                    ...
                    }}/>
                {
                    ...
                    <Button type="button" label={this.props.t('create_admin')} icon="pi pi-cog"
        ...
        const viewDetails = (rowData) => <div>
            <Button label={this.props.t('view') + ' ' + this.props.t('admin_details')} className="p-button-info p-button-raised"  style={{marginRight: '.5em'}} onClick={()=>{
        ...
        return (
            <div className="p-grid">
                        ...
                        <h1>{this.props.t('admins')}</h1>
                        {
                            this.state.loading ?
                                ...
                                <DataTable ref={(el) => this.dt = el} value={this.state.dataTableValue}
                                    ...
                                    <Column header={this.props.t('admin_details')}
                ...
                <Dialog header={this.props.t('admin_details')} visible={this.state.detailsShow} modal={true} style={{width: '40vw'}} footer={
                    <div>
                    ...
                    <div className="p-grid">
                        <div className="p-grid p-col-12 p-md-12">
                            <DataTable ref={(el) => this.dt = el} value={this.state.selectedFields} header={this.props.t('admin_fields')} selectionMode="single" paginator={true} rows={7}
...
export default withTranslation()(ManageAdmin)
