...
class TruckFields extends Component {
    ...
    checkPermission(permissions){
        return permissions.filter((each)=>each.route === '/truck-field').length > 0;
    }

    componentDidMount() {
        let permissions = JSON.parse(localStorage.getItem('daym-user')).permissions;
        if(this.checkPermission(permissions)){
            this.setState({methods: permissions.filter((each)=>each.route === '/truck-field')[0].methods});
        }else {
    ...
    handleDeleteUserField(rowData){
        ...
        this.truckFieldService.deleteTruckField(rowData._id).then( response => {
            ...
            this.growl.show({ severity: 'success', summary: this.props.t('success'), detail: 'Truck Field deleted Successfully!' });
            this.setState({
                dataTableValue: _dataTable,
                deleteLoading: false,
            })
        }).catch((err)=>{
            this.growl.show({ severity: 'error', summary: this.props.t('error'), detail: 'Can not delete Truck Field' });
    ...
    handleCreateUserField(){
        if(this.validate())
            ...
            if(this.state.editData){
                this.truckFieldService.editTruckField(this.state.editData._id,this.state.name,this.state.radioValue,this.state.chipsValue,this.state.required).then(async (response)=>{
                    this.growl.show({ severity: 'success', summary: this.props.t('success'), detail: 'Truck field edited Successfully!' });
                    let _dataTable = this.state.dataTableValue;
                ...
                this.truckFieldService.createTruckField(this.state.name,this.state.radioValue,this.state.chipsValue,this.state.required).then(async (response) => {
                    this.growl.show({severity: 'success', summary: this.props.t('success'), detail: 'Truck field created Successfully!'});
                    this.setState({
            ...
            <div className="p-grid">
                ...
                <div className="p-col-12">
                    <div className="card card-w-title datatable-demo">
                        <h1>{this.props.t('truck_fields')}</h1>
                        {
                            this.state.loading ?
                                <div className="p-col-12"
...
export default withTranslation()(TruckFields)
